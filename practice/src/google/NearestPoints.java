...
public class NearestPoints {
	public static double getAngle(Point a){
		double angle = Math.atan(a.y/a.x);
		return angle;
	}
	...
	private static void getSorted(PointAndAngle[] p){
	...
	public static Point[] getNearest(Point[] inside, Point[] outside){
		...
		getSorted(insideAngle);
		getSorted(outsideAngle);
		Point[] ret = new Point[2];
		double min = 0;
		for(int i=0; i<outside.length; i++){
			int left=0, right=inside.length, middle=(left+right)/2;
			for(; left<right; middle=(left+right)/2){
				if(insideAngle[middle].angle>outsideAngle[i].angle)	right = middle-1;
				else if(insideAngle[middle].angle==outsideAngle[i].angle) break;
				else left = middle+1;
			}
		...
		System.out.println(ret[0].x+", "+ret[0].y+" and "+ret[1].x+", "+ret[1].y);
		return ret;
	}
